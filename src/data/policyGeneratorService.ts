export type GeneratorLanguage = "en" | "ar";
...
export type GeneratedPolicyDraft = {
  detectedLanguage: GeneratorLanguage;
  topicId: string;
  title: string;
  titleAr: string;
  description: string;
...
const OPENAI_ENDPOINT = (import.meta.env.VITE_OPENAI_ENDPOINT as string | undefined)?.replace(/\/$/, "") ?? "https://api.openai.com/v1/chat/completions";
const OPENAI_MODEL = (import.meta.env.VITE_OPENAI_MODEL as string | undefined) ?? "gpt-4o-mini";

const IT_OPM = (title: string, titleAr: string, groupId: string, page: string, recordIds: string[]): PolicyReference => ({
...
export function detectGeneratorLanguage(value: string): GeneratorLanguage {
  const ar = (value.match(/[\u0600-\u06ff]/g) ?? []).length;
  const en = (value.match(/[a-z]/gi) ?? []).length;
  return ar >= en ? "ar" : "en";
}

function scoreTopic(prompt: string, topic: TopicDefinition): number {
  const q = normalize(prompt);
  return topic.terms.reduce((score, term) => {
    const n = normalize(term);
    if (!n) return score;
    if (q === n) return score + 6;
    if (q.includes(n)) return score + (n.includes(" ") ? 3 : 2);
    return score;
  }, 0);
}

function inferDocumentType(prompt: string, topic: TopicDefinition): string {
  const q = normalize(prompt);
  if (["procedure", "manual", "steps", "workflow", "اجراء", "خطوات", "دليل اجرائي", "كيفيه"].some(v => q.includes(normalize(v)))) return "Office Procedure Manual";
  if (["standard", "baseline", "معيار", "ضابط", "متطلبات فنيه"].some(v => q.includes(normalize(v)))) return "Standard";
  if (["guideline", "guide", "ارشادات", "ارشاد", "دليل استخدام"].some(v => q.includes(normalize(v)))) return "Guideline";
  if (["framework", "strategy", "اطار", "استراتيجيه"].some(v => q.includes(normalize(v)))) return "Framework";
  if (["support-ticket", "business-continuity", "change-management"].includes(topic.id)) return "Office Procedure Manual";
  if (topic.id === "access-security") return "Standard";
  return "Policy";
}


function titlesForDocumentType(topic: TopicDefinition, documentType: string): { title: string; titleAr: string } {
  const englishSuffix: Record<string, string> = {
    Policy: "Policy",
  ...
  const arabicPrefix: Record<string, string> = {
    Policy: "سياسة",
...
function referencesText(references: PolicyReference[], lang: GeneratorLanguage): string[] {
  if (!references.length) return [lang === "ar" ? "لا توجد مراجع داخلية معتمدة مرتبطة تلقائيًا بهذه المسودة؛ يجب على المراجع إضافة المراجع المطبقة قبل الاعتماد." : "No approved internal reference was automatically linked to this draft; the reviewer must add applicable references before approval."];
...
export function buildPromptAwareDraft(prompt: string, lang: GeneratorLanguage): GeneratedPolicyDraft {
...
function buildEnglish(topic: TopicDefinition, docType: string): string {
...
function buildArabic(topic: TopicDefinition, docType: string): string {
  const operational = docType === "Office Procedure Manual";
  const docTypeAr: Record<string, string> = {
    Policy: "السياسة",
...
async function openAIPolicyPrompt(prompt: string): Promise<string> {
...
function validateOpenAIResult(value: unknown): value is GeneratedPolicyDraft {
...
function localPolicyDraft(prompt: string): GeneratedPolicyDraft {
...
export async function generatePolicyDraft(prompt: string): Promise<GeneratedPolicyDraft> {
